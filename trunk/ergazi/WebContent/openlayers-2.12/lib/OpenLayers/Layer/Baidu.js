 ...
 */
OpenLayers.Layer.Baidu = OpenLayers.Class(
    OpenLayers.Layer.EventPane, 
    OpenLayers.Layer.FixedZoomLevels, {
    ...
    initialize: function(name, options) {
        options = options || {};
        options.version = "3";
        var mixin = OpenLayers.Layer.Baidu["v" + options.version];
        if (mixin) {
            OpenLayers.Util.applyDefaults(options, mixin);
        } else {
            throw "Unsupported Baidu Maps API version: " + options.version;
        }
    ...
    clone: function() {
         ...
         */
        return new OpenLayers.Layer.Baidu(
    ...
    display: function(visible) {
        if (!this._dragging) {
            this.setBMapVisibility(visible);
        }
    ...
    removeMap: function(map) {
        ...
        var cache = OpenLayers.Layer.Baidu.cache[map.id];
        if (cache) {
            if (cache.count <= 1) {
                this.removeBMapElements();
                delete OpenLayers.Layer.Baidu.cache[map.id];
            } else {
