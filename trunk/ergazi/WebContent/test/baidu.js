...
KoalaGIS.Layer.WMTS.Baidu = OpenLayers.Class(OpenLayers.Layer.XYZ, {
	...
	initialize : function(options) {
		...
		// this block steps through translating the values from the server layer
		// JSON
		...
		if (this.layerInfo) {
			...
			// esri uses slightly different IDs, so this may not be
			// comprehensive
			this.projection = 'EPSG:' + info.spatialReference.wkid;
			this.sphericalMercator = (info.spatialReference.wkid == 102100);
			// convert esri units into openlayers units (basic feet or meters
			// only)
			this.units = (info.units == "esriFeet") ? 'ft' : 'm';
			// optional extended section based on whether or not the server
			// returned
			// specific tile information
			if (!!info.tileInfo) {
				// either set the tiles based on rows/columns, or specific
				// width/height
	...
	getURL : function(bounds) {
		...
		// this.tileSize.h))));
		var x = (Math.round((center.lon - originTileX)
				/ (res * this.tileSize.w)));
		// var y = (Math.round(Math.abs((originTileY - center.lat) / (res *
		// this.tileSize.h))));
		var y = (Math.round((center.lat - originTileY)
				/ (res * this.tileSize.h)));
