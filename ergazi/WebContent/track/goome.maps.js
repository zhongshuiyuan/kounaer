(function(window) {
	...
	function Map(options) {
		...
		var sandbox = options.sandbox || defaults.sandbox, lang = options.lang
				|| defaults.lang, node = (typeof options.id === "string") ? document
		...
		if (isBMap) {
			...
			map.centerAndZoom(options.center
					|| new BMap.Point(defaults.lng.baidu, defaults.lat.baidu),
			...
			var opts = {
				...
				center : options.center
						|| new google.maps.LatLng(defaults.lat.google,
								defaults.lng.google),
	...
	Map.prototype.getDiv = function() {
		return this._source.getDiv ? this._source.getDiv() : this._source
				.getContainer()
	};
	...
	Map.prototype.getMapTypeId = function() {
		return this._source.getMapTypeId ? this._source.getMapTypeId()
				: this._source.getMapType()
	};
	...
	Map.prototype.fromLatLngToDivPixel = function(latlng) {
		if (this._source.pointToOverlayPixel) {
			return new goome.maps.Point(this._source
					.pointToOverlayPixel(latlng._source || latlng))
		} else {
			return new goome.maps.Point(overlay.getProjection()
					.fromLatLngToDivPixel(latlng._source || latlng))
		}
	};
	function LatLng(lat, lng) {
		if (arguments.length !== 1) {
			var k = objectMapping.LatLng[mapperKey];
			this._source = isBMap ? (new tempMaps[k](lng, lat))
					: (new tempMaps[k](lat, lng))
		} else {
			this._source = lat
		}
	}
	...
	LatLng.prototype.lat = function() {
		return (typeof this._source.lat === "function") ? this._source.lat()
				: this._source.lat
	};
	LatLng.prototype.lng = function() {
		return (typeof this._source.lng === "function") ? this._source.lng()
				: this._source.lng
	};
	function Size(w, h) {
		if (arguments.length === 1) {
	...
	function LatLngBounds(sw, ne) {
		if (arguments.length === 1) {
			...
			this._source = new tempMaps[objectMapping.LatLngBounds[mapperKey]](
					sw._source, ne._source)
		}
	}
	...
	function Circle(opts) {
		...
		var circle;
		if (isBMap) {
			circle = new tempMaps[objectMapping.Circle[mapperKey]](opts.center,
					opts.radius, opts)
		} else {
	...
	function Polyline(opts) {
		...
		opts.path = apiPath;
		if (isBMap) {
			polyline = new tempMaps[objectMapping.Polyline[mapperKey]](
					opts.path, opts)
		} else {
	...
	Polyline.prototype.getPath = function() {
		if (this._source.setOptions) {
			var arr = [], gRet = this._source.getPath(), len = gRet.getLength(), i = 0;
			for (; i < len; i++) {
	...
	Polyline.prototype.addPath = function(latlng, map) {
		...
		if (isBMap) {
			...
			(map && map._source || this._source.getMap())
					.addOverlay(this._source)
		}
	};
	...
	function Marker(opts, size, anchor) {
		...
		var position = opts.position._source || opts.position, marker;
		if (isBMap) {
			if (opts.icon) {
				var param;
				...
				if (size && size.width && size.height) {
					opts.icon = new BMap.Icon(opts.icon, new BMap.Size(
							size.width, size.height), param)
				} else {
					throw Error("Marker:parameter size with props width and height missed")
				}
			}
			marker = new tempMaps[objectMapping.Marker[mapperKey]](position,
					opts);
			if (opts.map) {
	...
	Marker.prototype.setIcon = function(url, size) {
		...
		if (this._source.setOptions) {
			...
			if (size && size.width && size.height) {
				this._source.setIcon(new BMap.Icon(url, new BMap.Size(
	...
	Marker.prototype.setDraggable = function(flag) {
		if (this._source.setDraggable) {
			...
			if (this._source.enableDragging) {
				flag ? this._source.enableDragging() : this._source
						.disableDragging()
			}
		}
	};
	...
	function InfoWindow(opts) {
		...
		if (isBMap) {
			this._source = new tempMaps[objectMapping.InfoWindow[mapperKey]](
					opts.content, opts)
		} else {
			this._source = new tempMaps[objectMapping.InfoWindow[mapperKey]](
					opts)
